...
//병원 리뷰작성하기
const HospitalReview = (props) => {

    const {navigation, userInfo, user_lang, route} = props;
    const {params} = route;

    console.log("params", params.idx);

    const [pageText, setPageText] = useState("");
    const [pageText2, setPageText2] = useState("");

    const pageTextApi = () => {
        Api.send('app_page', {'cidx':user_lang != null ? user_lang?.cidx : userInfo?.cidx, "code":"hospitalReview1"}, (args)=>{
        ...
        Api.send('app_page', {'cidx':user_lang != null ? user_lang?.cidx : userInfo?.cidx, "code":"hospitalReview2"}, (args)=>{
    ...
    const galleryOpen = () => {
        ...
        ImagePicker.openPicker({
            ...
            if(imageIndex != ""){
                ...
                imgs[imageIndexVal] = {uri:image.path, type:image.mime, data:image.data, name:'profile' + imageIndexVal + '.jpg'};
                setReviewImage(imgs);

            }else{
                image.map((item, index) => {
                    return imgs.push({uri:item.path, type:item.mime, data:item.data, name:'profile' + index + '.jpg'});
                })
    ...
    const cameraOpen = () => {
        ImagePicker.openCamera({
            ...
            if(imageIndex != ""){
                ...
                imgs.push({uri:image.path, type:image.mime, data:image.data, name:'profile' + imgLength + '.jpg'});
                setReviewImage(imgs);
            }
    ...
    const reviewWriteEvent = async () => {
        ...
        // Api.send('hospital_reviewWrite', {'id':userInfo?.id, 'cidx':user_lang != null ? user_lang?.cidx : userInfo?.cidx, 'hidx':params.idx, 'star':starIdx, 'content':reviewContent, 'addinfo1':qaAnswer, 'addinfo2':qaAnswer2, 'addinfo3':qaAnswer3, 'addinfo4':reVisit, 'addinfo5':hospitalPrice}, (args)=>{
        ...
        formData.append("method", "hospital_reviewWrite");
        formData.append("id", userInfo?.id);
        formData.append("cidx", user_lang != null ? user_lang?.cidx : userInfo?.cidx);
        formData.append("hidx", params.idx);
        ...
        if(reviewImage != ""){
            reviewImage.map((item, index) => {
                ...
                return formData.append('upfile[]', {'uri' : item.uri, 'name': fullName, 'type': item.type});
            })
        }
    ...
    return (
        <Box flex={1} backgroundColor='#fff' >
            ...
            }
            {
                ...
                <ScrollView>
                    ...
                    >
                        ...
                        >
                            ...
                            <Box>
                                <DefText 
                                    text={params.name}
                                    style={[fsize.fs15, fweight.bold, {color:'#191919', marginBottom:5}]}
                                />
                                <HStack>
                                    <StartIcons 
                                        onPress={()=>startSelectEvent(1)}
                    ...
                    >
                        ...
                        />
                        <ReviewQaBox 
                            qaTitle={ pageText2 != "" ? pageText2[1] : '선생님은 친절하셨나요?'}
                    ...
                    >
                        ...
                        >
                            <DefInput 
                                placeholder={ pageText2 != "" ? pageText2[4] : '최소 10자 이상 입력해 주세요.'}
                                value={reviewContent}
                    ...
                    >
                        ...
                        >
                            <DefText 
                                text={ pageText2 != "" ? pageText2[8] : "리뷰 사진을 올려주세요."}
                                style={[styles.reviewTitle]}
                            />
                            <DefText 
                                        ...
                                        reviewImage.map((item, index) => {
                                            return(
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <TouchableOpacity
                                                            style={[styles.cameraSelectButton, {borderWidth:0}]}
                    ...
                    >
                        ...
                        >
                            <DefText 
                                text={ pageText2 != "" ? pageText2[14] :  "알려주시면 큰 도움이 돼요."}
                                style={[styles.reviewTitle]}
                            />
                            <DefText 
                                text={ pageText2 != "" ? " (" + pageText2[9] + ")" : " (선택)"}
                        ...
                        <Box mt='15px'>
                            <DefInput 
                                placeholder={ pageText2 != "" ? pageText2[15] : '진료 비용을 입력해주세요.'}
                                value={hospitalPrice}
            ...
            }
            {
                page == 2 &&
                <DefButton 
                    text={ pageText2 != "" ? pageText2[16] : "작성완료"}
            ...
            </Modal>
            {
                uploadLoad &&
                <UploadLoading 
                loadingText={ pageText2 != "" ? pageText2[17] : "리뷰 작성 중 입니다\n잠시만 기다려주세요"}
                />
            }
        </Box>
    );
