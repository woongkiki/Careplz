...
const UntactReservationView = props => {
  ...
  const reservationCancleHandler = () => {
    Api.send(
      ...
      args => {
        ...
        if (resultItem.result === 'Y' && arrItems) {
          console.log('비대면 예약 취소 성공: ', resultItem, arrItems);
          ToastMessage(pageText[22]);
          setCancleModal(false);
          hospitalReservationApi();
        } else {
          console.log('비대면 예약 취소 실패!', resultItem);
        }
      },
    );
  };
              ...
              {reserInfo.status == 'RC' || reserInfo.status == 'RD' ? (
                <HStack alignItems={'center'} justifyContent="space-between">
                  <TouchableOpacity
                    ...
                    <DefText text="병원정보" style={[styles.buttonText]} />
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[
                      styles.button,
